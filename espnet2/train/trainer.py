...
class Trainer:
    ...
    def train_one_epoch(
        ...
        print("Iterator_stop: \t", iterator_stop)
        start_time = time.perf_counter()
    ...
    def plot_attention(
        ...
        for ids, batch in iterator:
            ...
            print("Getting ESN State for logging")
            att_dict = calculate_all_attentions(model, batch)
            if model.encoder.get_input_layer() == "echo":
                log_echo = True
                esn_state = model.encoder.get_esn_state().detach().cpu().numpy() if isinstance(model.encoder.get_esn_state(), torch.Tensor) else model.encoder.get_esn_state().numpy()
            else:
                log_echo = False
            ...
            if log_echo:
                ...
                plt.axes(xlabel='Window Step (time)', ylabel='ESN State', title='Transformer Frontend ESN State over Time')
                img = plt.imshow(esn_state[0][0], cmap='gnuplot')
                if output_dir is not None:
                    esn_p = output_dir / ids[0] / f"esn_state.{reporter.get_epoch()}ep.png"
                    esn_p.parent.mkdir(parents=True, exist_ok=True)
                    fig.savefig(esn_p)
                if summary_writer is not None:
                    summary_writer.add_figure(f"esn_state.{0}",fig, reporter.get_epoch())
